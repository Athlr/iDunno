...
class FriendRequestsRepo(BaseModel):
    ...
    def create_friend_request(
        self, user_id: int, friend_request: MakeFriendRequestIn
    ) -> Union[MakeFriendRequestOut, Error]:
        try:
            with pool.connection() as conn:
                with conn.cursor() as db:
                    ...
                    if exists.fetchone()[0] > 0:
                        return {
                            "message": "Friend request has already been made to this user"
                        }
                    else:
                        result = db.execute(
                            """
    ...
    def update_friend_request(
        self, receiver_id: int, request_id: int, friend_request: UpdateFriendRequest
        ...
        try:
            with pool.connection() as conn:
                with conn.cursor() as db:
                    ...
                    if exists and (
                        request_id == exists[0] and receiver_id == exists[3]
                    ):
                        if (
                            exists[1] == "pending"
                            ...
                            db.execute(
                                ...
                                """,
                                {"user1_id": exists[2], "user2_id": receiver_id},
                            )

                            return UpdateFriendRequest(status="accepted")
                        elif (
                            exists[1] == "pending"
                            and friend_request.status == "rejected"
                        ):
                            db.execute(
                                """
                            ...
                            return UpdateFriendRequest(status="rejected")
                        else:
